    ...
    const DashboardView = ({ userRole }) => {
        ...
        const handleSaveKpi = async (assignedKpiDocId, newValue, udyamMitraId) => {
            ...
            try {
                // IMPORTANT: When deployed with SWA, API calls go to /api
                const updateKpiUrl = '/api/UpdateKpiSubmission';

                // Get Firebase ID token for authentication and authorization on Azure Function
                ...
                const response = await fetch(updateKpiUrl, {
                    ...
                    headers: {
                        ...
                        'Authorization': `Bearer ${idToken}` // Pass ID token in header
                    },
                    body: JSON.stringify({
                        assignedKpiDocId: assignedKpiDocId, // Pass the Firestore document ID to the function
                        submittedValue: newValue,
                        udyamMitraId: udyamMitraId, // This is already in the userKpi document, but passed for consistency
                ...
                console.log('Submission recorded successfully via Azure Function.');
                // Firestore listener will automatically update the UI after the Function updates Firestore
                return true;

            } catch (e) {
                console.error("Error submitting KPI update via Azure Function:", e);
